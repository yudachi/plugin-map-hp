...
export const reactClass = connect(
      ...
      case '/kcsapi/api_get_member/mapinfo': {
        for (const mapInfo of body) {
          if (mapInfo.api_eventmap) {
            ...
            mapHp.push([mapInfo.api_id, currentHp, mapInfo.api_eventmap.api_max_maphp])
          } else {
            if (typeof $maps[mapInfo.api_id].api_required_defeat_count != "undefined" && $maps[mapInfo.api_id].api_required_defeat_count !== null) {
              ...
              mapHp.push([mapInfo.api_id, currentHp, $maps[mapInfo.api_id].api_required_defeat_count])
            }
          }
        }
        this.setState({ mapHp: mapHp })
      }
    }
  }
  handleSetClickValue = () => {
  ...
  render() {
    let mapHp = []
    for (const mapInfo of this.state.mapHp) {
